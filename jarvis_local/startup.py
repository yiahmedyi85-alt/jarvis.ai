...
def set_startup(enabled:bool,launcher:str):
    ...
    try:
        ...
        shell=win32com.client.Dispatch("WScript.Shell"); shortcut=shell.CreateShortCut(str(link)); shortcut.TargetPath=sys.executable; shortcut.Arguments=f'"{launcher}"'; shortcut.WorkingDirectory=str(Path(launcher).parent); shortcut.Save()
